...
#
# **Five draws was still too few, and the A8 experiment found out the expensive
# way.** The gate ran at `DENSE_SAMPLES = 5, DENSE_MIN_EXACT = 4` and went red on
# `computer-work-ni-ne` the first time that work re-recorded — 3/5, read as a
# regression the prompt cut had caused. Measured at twenty draws it was 16/20 on
# the cut prompt and 17/20 on the one shipped: no regression, and a gate that
# false-fails about a quarter of the time at the rate this case actually has.
# The prompt cut itself was declined (PR #102, closed); this depth is the part
# of it worth keeping, and it is a test-only change. On A6.6's encoding the same
# case draws 19/20 — two draws from 17/20, which is to say the same rate.
